...
def load_hifigan(config, checkpoint_path="./hifigan/g_00205000", device="cuda"):
    ...
    if checkpoint_path.startswith("http"):
        ckpt = (
            ...
            if device != "cuda"
            else torch.hub.load_state_dict_from_url(checkpoint_path)
        )
    else:
        ckpt = (
...
class MaskCycleGANVCTesting(object):
    ...
    def __init__(self, args):
        ...
        self.converted_audio_dir = os.path.join(
            args.save_dir, args.name, args.converted_audio_subdir
        )
        ...
        # hifi
        elif args.vocoder == "hifi":
            self.model_config = yaml.load(
                open("./hifigan/my_model.yaml", "r"), Loader=yaml.FullLoader
            )
